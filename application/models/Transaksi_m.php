...
<<<<<<< HEAD
        ...
        // $query = "SELECT pembayaran.*, sum(pembayaran.jumlah) as jumlah_total, count(pembayaran.jumlah) as jumlah_bulan_bayar, kk.nomor_id, kk.nama_kk, kk.alamat 
        //             FROM pembayaran INNER JOIN kk ON kk.id_keluarga = pembayaran.id_keluarga 
        //             WHERE tglbayar BETWEEN date_format('$mulaiTgl', '%d-%m-%Y') AND date_format('$sampaiTgl', '%d-%m-%Y') 
        //             GROUP BY pembayaran.id_keluarga";
        // return $this->db->query($query);
        
        $query = "SELECT pembayaran.*, SUM(pembayaran.jumlah) AS jumlah_total, COUNT(pembayaran.jumlah) AS jumlah_bulan_bayar, kk.nomor_id, kk.nama_kk, kk.alamat 
                  FROM pembayaran 
                  INNER JOIN kk ON kk.id_keluarga = pembayaran.id_keluarga 
                  WHERE STR_TO_DATE(tglbayar, '%d-%m-%Y') 
